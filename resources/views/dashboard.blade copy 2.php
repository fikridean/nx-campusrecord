...
<body class="bg-gray-100">
    <div class="flex h-screen overflow-hidden">
        @can('isAdmin')
            <div id="sidebar"
                class="w-64 bg-blue-700 text-white flex flex-col min-h-screen fixed transform -translate-x-full transition-transform duration-200 ease-in-out">
                ...
                <div class="pl-7">
                    <nav class="flex-1">
                        <ul class="space-y-5">
                            <li id="navDataAdmin"
                                class="p-4 hover:text-blue-800 hover:bg-white hover:rounded-tl-full hover:rounded-bl-full hover:font-bold active-nav-link">
                                <a href="javascript:void(0)" class="block">Data Admin</a>
                            </li>
                            <li id="navDataUser"
                                class="p-4 hover:text-blue-800 hover:bg-white hover:rounded-tl-full hover:rounded-bl-full hover:font-bold">
            ...
            @can('isAdmin')
                <div class="flex flex-col w-full p-10 bg-white overflow-x-auto" id="editAdmin">
                    <div class="my-5 flex">
                        ...
                        <button
                            class="mx-5 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Search</button>
                    </div>
                    <div class="bg-white p-6 rounded-lg shadow w-full overflow-x-auto max-h-screen h-fit">
                        ...
                        <button
                            class="bg-blue-600 text-white px-2 py-2 mb-4 rounded hover:opacity-80 {{ request()->is('tambahuser') ? 'active-nav-link' : '' }}">
                            + Tambah Admin
                        </button>
            ...
            @can('isStudent')
                <div class="flex flex-col w-full p-10 bg-white overflow-auto">
                    ...
                    <div class="my-5 w-full flex">
                        <form action="" class="w-full flex" method="POST">
                            @csrf
                            <input id="search-input" type="text"
                                placeholder="Search your friend here by username / NIM / Name"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        ...
                        <table class="min-w-full bg-white">
                            ...
                            <tbody id="results-table-body">
                                ...
                                @else
                                    @if (isset($users))
                                        <tr>
                                            <td class="py-2 px-4 border-b text-center">No users found matching your search
                                                criteria.</td>
                                        </tr>
                                    @endif
                                @endif
                ...
                success: function(data) {
                    ...
                    last30Activities.forEach(function(activity) {
                        var row = '<tr>' +
                            ...
                            '<td class="py-2 px-4 border text-center">' + activity.activity +
                            '</td>' +
                            '<td class="py-2 px-4 border text-center">' + activity.created_at +
                            '</td>' +
                            '<td class="py-2 px-4 border text-center">' + activity.updated_at +
                            '</td>' +
                            '</tr>';
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        response.data.forEach(function(user) {
                            var row = '<tr>' +
                                '<td class="py-2 px-4 border text-center">' + number +
                                '</td>' +
                                '<td class="py-2 px-4 border text-center">' + user
                                .username + '</td>' +
                                '<td class="py-2 px-4 border text-center">' + user.nim +
                                '</td>' +
                                '<td class="py-2 px-4 border text-center">' + user
                                .name + '</td>' +
                                '</tr>';
